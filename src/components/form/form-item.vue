<template>
    <div :class="classes">
        <label :class="[prefixCls + '-label', 'j-form-item-tit']" :for="labelFor" :title="strLen(label||longLabel)>33?label||longLabel:''" :style="labelStyles" v-if="label || longLabel || $slots.label"><slot name="label">{{ currentLabel }}{{ FormInstance.colon }}</slot></label>
...
<script>
    ...
    import {strLen} from '../../libs/api.utils';
    ...
    export default {
        ...
        computed: {
            ...
            currentLabel() {
                return (this.label || this.longLabel) + this.labelSuffix
            }
        },
        methods: {
            setRules() {
