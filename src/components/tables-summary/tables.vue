<template>
    <div :class="clz">
        <div v-if="searchable || btnOnly" class="search-con-parent">
            <div class="search-con search-con-top">
                <Card class="search-card">
                    <Form ref="condForm" :label-width="labelWidth" class="search-con search-con-top j-table-form" inline
                        ...
                        <Row>
                            ...
                            <Col :span="col">
                                <div class="fl-r search-btn">
                                    <Button v-if="searchable" class="j-search-btn" @click="handleSearch"
                                            :loading="loading" type="primary" shape="circle"
                                            icon="ios-search"></Button>
                                    <Button v-if="searchable" class="j-search-btn" @click="handleRefresh"
                                            :loading="loading" type="primary" shape="circle"
                    ...
                    <Row>
                        <Col span="24">
                            <slot v-if="isShowData" name="show-data"></slot>
                        </Col>
                    </Row>
                </Card>
            </div>
        ...
        <Card v-show="isShowTable" class="table-card" :class="{ 'no-search': !searchable && !btnOnly }">
            <h4 class="j-table-title" v-if="tableTitle">
                {{ tableTitle }}
            ...
            <div v-if="pageable" class="page-card-container">
                <div class="fl-r">
                    <Page :total="total" :current="currentPage" :page-size="currentPageSize" @on-change="changePage"
                          @keydown.native.enter.prevent="onSize(e.target.value)" @on-page-size-change="onSize"
...
<script>
    import {Card, Form, Row, Col, Button, Table, Page, Tooltip, Poptip, Icon} from 'view-design'
        ...
        methods: {
            handleSummary({columns, data}) {
                ...
                columns.forEach((column, index) => {
                    ...
                    if (this.summaryKill.includes(index)) {
                        sums[key] = {
                            key,
                            value: ''
                        }
                    ...
                    if (!values.every(value => isNaN(value))) {
                        ...
                        let cloneV = v
                        if(!this.summaryInt.includes(index)) {
                            // cloneV = cloneV.toFixed(2)
                            cloneV = ApiUtils.formatMoney(cloneV)
                        }
                        sums[key] = {
                            key,
                            value: cloneV
            ...
            jSupportEdit(item, index) {
                item.render = (h, params) => {
                    return h('div', {
                        ...
                        on: {
                            blur: evt => {
                                evt.target.innerText = evt.target.innerText || params.row[params.column.key]
                ...
                item.render = (h, params) => {
                    if (params.row.pure) {
                        ...
                        return h('Input', {
                            ...
                            on: {
                                'on-blur': (evt) => {
                                    this.insideTableData[params.index][params.column.key] = evt.target.value
            ...
            jSupportSelect(item, index) {
                item.render = (h, params) => {
                    if (params.row.pure) {
                        return h('span', {}, item.tableDict.val2Nm[params.row[params.column.key]])
                    } else {
                        return h('Select', {
                            props: {
                                value: params.row[params.column.key],
            ...
            surportHandle(item) {
                let insideBtns = ApiUtils.buildTableBtns(item.options, this.authCode)
            ...
            surportType(item) {
                item.render = (h, params) => {
                    return h('span', valueToText(params.row[item.key], item.type, item.code))
                }
...
//                    if (res.editable) res = this.suportEdit(res, index)
                    ...
                    if (res.type) res = this.surportType(res)
                    return res
                })
            },
            setDefaultSearchKey() {
